...
def read_command(command, board):
    ...
    while True:
        try:
            temp_x, temp_y = input('Please enter the open square(example: a,1)').split(',')
...
def postprocess_game(clear_time):
    if not clear_time is None:
        print('GAME CLEAR!!')
        print('clear time:' + str(round(clear_time)) + '[s]')
    else:
        print('GAME OVER')
